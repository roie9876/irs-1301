...
- education_fund: הפרשת המעסיק לקרן השתלמות - שדה 219/218 (מספר). חשוב: חפש את "הפרשת המעסיק לקרן השתלמות" שזה הסכום שהמעסיק הפקיד בפועל. אל תיקח את "משכורת לצורך הפקדות לקרן השתלמות" כי זה הבסיס לחישוב ולא ההפקדה עצמה. לדוגמה: אם כתוב "משכורת לצורך הפקדות לקרן השתלמות 132,576" ו"הפרשת המעסיק לקרן השתלמות 9,943" — הערך הנכון הוא 9,943
...
FORM_867_EXTRACTION_PROMPT = """אתה מומחה לחילוץ נתונים מטופס 867 ישראלי (אישור ניכוי מס במקור — דיבידנד וריבית מניירות ערך, או ריבית מפקדונות ותוכניות חיסכון).
חלץ את השדות הבאים מהטקסט שלהלן והחזר JSON בלבד.
...
- שנת המס: חפש "לשנת המס XXXX" — קח את השנה האחרונה שמופיעה ליד "לשנת המס". אם מופיע גם 2021 וגם 2025, קח 2025.

השדות לחילוץ:
- broker_name: שם הברוקר/חייב/בנק (טקסט)
- broker_id: מספר תיק ניכויים (טקסט). זה מספר של 9 ספרות ליד "תיק ניכויים" או "מספר תיק ניכויים". אל תיקח ת.ז. של שותפים בחשבון
- account_name: שם החשבון (טקסט)
- tax_year: שנת מס (מספר). חשוב: קח את שנת המס מ"לשנת המס XXXX" — השנה העדכנית ביותר
...
- interest_income: סה"כ הכנסה מריבית — סכום כל שורות הריבית (שורה 7 או שדות 078+076+126+142+053). null אם אין ריבית
- interest_tax_withheld: סה"כ מס שנוכה במקור מריבית — שורה 11 או שדה 043 (מספר)
- interest_15: הכנסה מריבית בשיעור 15% — שדה 078 (מספר). null אם לא מופיע
- interest_20: הכנסה מריבית בשיעור 20% — שדה 126 (מספר). null אם לא מופיע
- interest_25: הכנסה מריבית בשיעור 25% — שדה 142 (מספר). null אם לא מופיע
...
- tax_year: שנת המס שאליה מתייחסת הקבלה (מספר). אם כתוב "הגשת דוח 2022" — tax_year=2022. אם לא ניתן לזהות, החזר null.
...
מבנה ספח תעודת זהות:
הספח מכיל 3 אזורים מובחנים:
1. **אזור בעל התעודה** (בראש הדף): שם משפחה, שם פרטי, שם האב, מספר זהות, תאריך לידה, מין.
   - holder_name = שם משפחה + שם פרטי (בלבד). אסור לכלול את שם האב!
   - שם האב מופיע בשדה נפרד (לרוב "שם האב") — אל תכלול אותו ב-holder_name.
2. **אזור בן/בת הזוג**: שם, מספר זהות, תאריך נישואין.
3. **אזור הילדים** (טבלה): שם, מספר זהות, תאריך לידה לכל ילד.

כללים קריטיים:
- holder_name חייב להכיל רק שם משפחה + שם פרטי. לדוגמה: אם כתוב שם משפחה "בן חיים", שם פרטי "רועי", שם האב "זאב" — holder_name = "בן חיים רועי" (ולא "בן חיים רועי זאב").
